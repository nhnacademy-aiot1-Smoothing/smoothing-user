...
public class UserController {
    ...
    @GetMapping("/login")
    public ResponseEntity<UserResponseTemplate> getUserSimpleInfo(@RequestHeader("X-USER-ID") String userId) {

        return ResponseEntity.ok(userService.getUserSimpleInfo(userId));
    }

    @GetMapping("/profile")
    public ResponseEntity<UserResponseTemplate> getUserDetailInfo(@RequestHeader("X-USER-ID") String userId) {

        return ResponseEntity.ok(userService.getUserDetailInfo(userId));
    }

    @PatchMapping("/profile")
    public ResponseEntity<String> userInfoModify(@RequestHeader("X-USER-ID") String userId, @RequestBody UserInfoModifyRequest request) {
        userService.modifyUserInfo(userId, request);
        return ResponseEntity.ok("사용자 비밀번호 변경 완료");
    }

    @PatchMapping("/profile/password")
    public ResponseEntity<String> userPasswordModify(@RequestHeader("X-USER-ID") String userId, @RequestBody UserPWModifyRequest request) {
